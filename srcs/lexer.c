...
/*   Updated: 2024/11/12 01:21:22 by zuzanapiaro      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *get_node_value(char *input, int *i, int treat_specials_as_text_dq, int treat_specials_as_text_sq, int start)
{
	...
	node_value = NULL;
	if (treat_specials_as_text_sq == 0 && treat_specials_as_text_dq == 0
	...
	else if (treat_specials_as_text_sq == 0 && treat_specials_as_text_dq == 1
		&& (input[*i]  == '"'))
		node_value = ft_substr(input, start, *i - start + 1);
	else if (treat_specials_as_text_sq == 1 && treat_specials_as_text_dq == 0
		&& (input[*i] == '\''))
		node_value = ft_substr(input, start, *i - start + 1);
	return (node_value);
}
...
char	*process_text(char *input, int *i)
{
	...
	while (input && input[(*i)])
	{
		...
		else if (in_sq == 0 && in_dq == 0 && (iswhitespace(input[*i])
			|| input[*i] == '>' || input[*i] == '<' || input[*i] == '|'))
		{
			node_value = ft_substr(input, start, *i - start);
			(*i)--;
		}
		else if ((in_sq == 0 && in_dq == 1 && (input[*i]  == '"'))
			|| (in_sq == 1 && in_dq == 0 && (input[*i] == '\'')))
			node_value = ft_substr(input, start, *i - start + 1);
		if (node_value)
			return (node_value);
		(*i)++;
